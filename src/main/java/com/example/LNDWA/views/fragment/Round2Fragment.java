...
import com.example.LNDWA.adapters.TabsPagerAdapter;
import com.example.LNDWA.cards.*;
import com.example.LNDWA.util.Round2Enum;
import com.example.LNDWA.util.Triple;
import com.example.LNDWA.util.Tuple;
import com.example.LNDWA.util.Utils;
import com.example.LNDWA.util.parser.GameParse;
import com.example.LNDWA.util.parser.PlayerParse;
import com.example.LNDWA.views.DialogEnum;
import com.example.LNDWA.views.Results;
...
public class Round2Fragment extends Fragment {
    ...
    private Integer calculateDestinationView(Integer changeAmount, final Integer position, final Boolean clockwise) {
        ...
        Log.e("calculateDestinationView", (position + destination) % this.relviewlist.size() + "");
        ...
        if (!this.lastactivecharacterrel.isEmpty() && !this.lastrelcolor.isEmpty()) {
            ...
            for (RelativeLayout rel : this.lastactivecharacterrel) {
                ...
                if (this.lastrelcolor.get(i) == Color.TRANSPARENT) {
                    rel.setBackgroundResource(R.color.darkdblue);
                    rel.setTag(R.color.darkblue);
                    i++;
                } else {
    ...
    private void executeRoundEnd() {
        ...
        if (!this.lastactivecharacterrel.isEmpty()) {
            for (RelativeLayout rel : this.lastactivecharacterrel) {
                int i = 0;
                Log.e("Color ", this.lastrelcolor.get(i) + " " + Color.TRANSPARENT);
                if (this.lastrelcolor.get(i) == R.color.darkdblue) {
        ...
        for (Karte card : this.cardlist) {
            if (end) {
                ...
                Collections.sort(this.cardlist, new Comparator<Karte>() {
                ...
                Log.e("Winnerlist - Winningcards", winnercards.size() + " - " + winnerlist.size());
    ...
    private List<Karte> getCardsArray(boolean loadgame) {
        ...
        Collections.sort(cardarray, new Comparator<Karte>() {
            public int compare(Karte s1, Karte s2) {
                return s1.getName().compareTo(s2.getName());
            }
        });
    ...
    public List<Karte> getDeadCardsArray() {
        ...
        Collections.sort(deadcardarray, new Comparator<Karte>() {
            public int compare(Karte s1, Karte s2) {
                return s1.getName().compareTo(s2.getName());
            }
        });
    ...
    public List<Karte> getExtraCardsArray() {
        ...
        Collections.sort(extracardarray, new Comparator<Karte>() {
            public int compare(Karte s1, Karte s2) {
                return s1.getName().compareTo(s2.getName());
            }
        });
    ...
    public String[] getGameLog() {
        return this.gamelog.toArray(new String[this.gamelog.size()]);
    }
    ...
    public void goBackOneStep() {
        if (counter > 0) {
            ...
            while (counter > 0 && (this.extracardlist.contains(currentcard) || currentcard.isdead() || (currentcard.getRound() != 0 && !currentcard.getRound().equals(rounds)
                ...
                Log.e("CurrentCard.getRound()!=0?", (currentcard.getRound() != 0) + " " + currentcard.getRound());
                Log.e("!CurrentCard.getRound().equals(rounds)?", (!currentcard.getRound().equals(rounds)) + " " + rounds);
                Log.e("!CurrentCard.getRound().equals(rounds*=-1)?", (!currentcard.getRound().equals(rounds * -1)) + " " + rounds * -1);
    ...
    public void killCharacter(Integer relid) {
        ...
        if (this.executeOnDeadAbilities(current, relid)) {
            ...
            if (current.getCurrentamount() > 1) {
                ...
                Log.e("Set  dead status to true for ", relid + "");
    ...
    public void onCreate(final Bundle savedInstanceState) {
        ...
        this.choosePlayer = new View.OnClickListener() {
            @Override
            public void onClick(final View view) {
        ...
        View.OnClickListener chooseCharacter = new View.OnClickListener() {
        ...
        if (!turn) {
            ...
            holder.showeventbutton.setVisibility(GONE);
            holder.showeventbutton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(final View view) {
            ...
            holder.showgamelogbutton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(final View view) {
                    Utils.dialogtext = "";
...
//        Log.e("Lastactivecharacterrel", this.lastactivecharacterrel.iterator().next().toString());
    ...
    private void switchLivingCharWithChar(final Karte current, final Karte toChange, final Integer relId, final Integer relId2) {
        Log.e("SwitchLivingCharWithChar", current.toString() + " " + toChange.toString() + " " + relId.toString() + " " + relId2.toString());
